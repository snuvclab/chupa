...
def visualize_masks(views, save_path: Path=None):
    """ Visualize a list of masks by plotting their masks.

    Args:
        views: The views to visualize.
        save_path (optional): Path to save the figure to.
    """
    masks = [v.mask.cpu() for v in views]

    fig, axs = create_mosaic_figure(masks) 
